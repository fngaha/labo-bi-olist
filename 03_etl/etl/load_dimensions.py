...
def load_d_category(dim_category: pd.DataFrame, truncate: bool = False) -> None:
    ...
    with engine.begin() as conn:
        if truncate:
            conn.execute(text("ALTER TABLE dbo.D_Product NOCHECK CONSTRAINT ALL;"))
            conn.execute(text("DELETE FROM dbo.D_Category;"))
...
def load_d_product(dim_product: pd.DataFrame, truncate: bool = True) -> None:
    ...
    with engine.begin() as conn:
        if truncate:
            conn.execute(text("ALTER TABLE dbo.F_Ventes_Items NOCHECK CONSTRAINT ALL;"))
            conn.execute(text("DELETE FROM dbo.D_Product;"))
...
def load_d_customer(dim_customer: pd.DataFrame, truncate: bool = True) -> None:
    ...
    with engine.begin() as conn:
        if truncate:
            conn.execute(text("DELETE FROM dbo.D_Customer;"))
...
def load_d_seller(dim_seller: pd.DataFrame, truncate: bool = True) -> None:
    ...
    with engine.begin() as conn:
        if truncate:
            conn.execute(text("DELETE FROM dbo.D_Seller;"))
...
def load_d_payment_type(dim_payment_type: pd.DataFrame, truncate: bool = True) -> None:
    ...
    with engine.begin() as conn:
        if truncate:
            conn.execute(text("DELETE FROM dbo.D_PaymentType;"))
...
def load_d_order_status(dim_order_status: pd.DataFrame, truncate: bool = True) -> None:
    ...
    with engine.begin() as conn:
        if truncate:
            conn.execute(text("DELETE FROM dbo.D_OrderStatus;"))
